...
// 시계도, 전역 상태도 없다.
//
// # 회차 상한이 2026-08-17 에 바뀌었다 (사용자 결정)
//
// 그 전까지 이 패키지가 지키는 것은 한 줄이었다: 회차당 최대 명목은
// equity × 0.0455 **미만**. 지금 회차 상한은 확신도가 정하는 절대값이고
// (stake.go 의 [StakeTarget], 최대 [MaxStakeUSD]), equity 비례 상한은
// **크기 결정에서 빠졌다.**
//
// 왜 뺐나: 자본 65 USDT 에서 [Cap] 은 2.96 USDT 였고, 그 값이 확신도별 목표
// (1.89~10.00)의 위쪽 다섯 칸을 통째로 눌러 크기 차등이 두 단계로 무너졌다.
// 차등을 넣은 이유가 그것이므로 상한을 뺐다.
//
// **무엇이 사라졌는지 분명히 해 둔다.** 이제 자본이 줄어도 베팅은 줄지 않는다.
// 남은 끈은 [StakeRemaining] 의 가용잔고 검사뿐이고, 그것은 "있는 돈보다 많이
// 걸지 않는다" 이지 "자본의 몇 %만 건다" 가 아니다.
//
// [Cap]·[Remaining]·[CanArm] 은 남아 있다 — 무장 판정과 진단·스냅샷이 쓴다.
// **크기 결정 경로에서는 부르지 않는다.**
//
// 한도를 넘길 수 있는 모든 경로 — 노출 누락, 올림, 시가 평가, 망가진 입력 —
// 를 여기서 한꺼번에 막는 것은 그대로다.
//
// # 망가진 입력에서의 방향
//
...
// CapFraction 은 회차당 최대 명목의 equity 대비 비율이다.
//
// **2026-08-17 부터 크기 결정에 쓰이지 않는다**(패키지 주석 참고). [Cap] 을
// 통해 무장 판정([CanArm])과 진단 스냅샷에만 남아 있다.
...
//
// **크기 결정과는 다른 기준을 쓴다**(2026-08-17 이후). 크기는 [StakeTarget]
// 이 정하고 [StakeRemaining] 이 가용잔고로만 자르므로, 이 함수가 요구하는
// equity 약 $22 는 그보다 엄격한 별도의 무장 문턱이다. 자본이 그 아래면
// 사이저는 주문을 만들 수 있어도 봇은 무장하지 않는다 — 의도된 보수성이다.
//
