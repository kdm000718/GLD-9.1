...
// 재시도마다 책을 다시 읽으면 원장의 어느 줄이 어느 호가에 근거했는지 사후에
// 말할 수 없다.
//
// 그래서 이 봇에는 **관통 방지가 없다.** 최우선 매수호가에 서는 것은 정의상
// 수동 주문이지만(오더북 1,714회차에서 관통 0건), 우리 주문이 도착하기 전에
// 매도호가가 내려오면 관통한다. 그것을 알고 고른 설계다(2026-08-12 사용자 결정).
//
// # 역선택 — 이 전략의 유일한 진짜 문제
//
// 지정가에 걸어 두면 **시장이 우리 쪽으로 내려올 때만** 체결되는데, 그것은 곧
// 우리 방향이 지고 있다는 뜻이다. 오더북 원본 1,406회차(07-29~08-06) 실측:
//
//	P(체결 | 우리가 이길 회차)   85.5%
//	P(체결 | 우리가 질 회차)     99.9%
//	질 회차의 평균 체결 여유가 이길 회차의 4.3배 (상관 −0.77)
//
// **질 때만, 그리고 질 때 더 많이 체결된다.** 이것이 이 전략의 손익을 지배한다.
// 손익분기 판정 승률은 체결분 기준(진입가/0.98)이 아니라
//
//	j* = P(체결|패)·px / ( P(체결|패)·px + P(체결|승)·(0.98−px) )
//
// 이고, 진입가 0.4937 에서 54.24% 다 — 체결분 기준 50.38% 보다 3.9%p 높다.
// **주문을 키우면 지는 쪽만 커져** j* 가 더 올라간다(명목 50 USDT 에서 55.6%,
// 200 USDT 에서 57.7%).
//
// # 가격 정책의 이력
//
//	0.47 고정   ~2026-08-14   238회차: 체결 90.8%, 판정 54.2%, 체결분 49.5%
//	0.46 고정    2026-08-14   124회차: 체결 80.6%, 판정 48.4%, 체결분 36.0%
//	0.48 고정    2026-08-14   사용자 결정 (가격을 낮출수록 낙차가 커졌으므로 반대로)
//	최우선호가   2026-08-16~  사용자 결정
//
// 고정가는 역선택을 피하지 못했다. 최우선호가로 옮기면 체결률이 87.7% → 95.6%
// 로 오르고 체결분 승률도 함께 오른다 — 대가는 평균 진입가 0.4787 → 0.4965 다.
//
...
type roundState struct {
	...
	// 테이커 다리(최우선 매도호가)와 메이커 다리(최우선 매수호가). 걸린 순서대로
...
// 정의상 즉시 관통해 먼저 체결된다. 메이커 다리는 시장이 우리 지정가까지
...
// 매도호가가 우리 지정가 이하였다면 우리도 관통해 테이커 수수료를 문 것이고,
