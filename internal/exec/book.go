...
//
// # 지금 이 파일이 하는 일 — 기록이고, 한 자리에서는 판단이다
//
// 2026-08-12 에 전략이 바뀌었다. 이 봇은 군중을 따라가지도, 관통을 피하지도
// 않는다. 그때부터 여기서 만든 호가창은 **어떤 결정에도 들어가지 않았다** —
// 주문을 낸 순간의 시장 모습을 로그 한 줄로 남기는 데만 썼다.
//
// **2026-08-14 에 예외가 하나 생겼다.** 회차마다 다리를 둘 걸게 됐고, 그중
// 테이커 다리의 가격은 여기서 읽은 최우선 매도호가 그대로다(exec.go 의
// [takerTick]). 테이커 가격은 정의상 시장이 정하는 값이라 상수로 만들 수 없다.
//
// 그래서 거울의 무게가 오히려 늘었다. 예전에는 거울이 틀리면 **기록**이
// 틀렸지만, 이제는 틀린 거울이 곧 **틀린 가격에 실제로 매수**다. 위의 2026-08-11
// 사고가 정확히 그 형태였다 — 그때는 관통 방지가 거울 없이 판단했고, 지금은
// 테이커 가격이 그렇다.
//
// **[takerTick] 말고 이 파일의 값을 읽어 가격이나 수량을 바꾸는 경로가 생기면
// 그것은 또 다른 전략 변경이다.** book_test.go 의 TestBookNeverChangesTheMakerLeg
// 가 메이커 다리를 지키고, TestTakerLegFollowsTheAsk 가 테이커 다리는 반대로
// 책을 따라야 함을 못 박는다.
