...
// 않는다. 그때부터 여기서 만든 호가창은 **어떤 결정에도 들어가지 않는다** —
// 주문을 낸 순간의 시장 모습을 로그 한 줄로 남기는 데만 쓴다.
//
// 2026-08-14 에 잠깐 예외가 있었다. 테이커 다리의 가격을 여기서 읽은 최우선
// 매도호가로 정했다(exec.go 의 takerTick). 같은 날 그 다리를 제거하면서 예외도
// 사라졌고, 이 파일은 다시 기록 전용이다.
//
// **이 파일의 값을 읽어 가격이나 수량을 바꾸는 경로가 생기면 그것은 전략
// 변경이다.** book_test.go 의 TestBookNeverChangesTheOrder 가 그것을 못 박는다 —
// 어떤 호가창 모양에서도 나가는 주문의 틱과 수량이 같아야 한다.
//
// 그래도 거울은 여전히 중요하다. 틀린 거울은 **틀린 기록**이고, 그 기록으로
// "그때 그 가격이 좋았나" 를 사후에 정반대로 답하게 된다 — 위의 2026-08-11
// 손실이 정확히 그 착각에서 나왔다.
