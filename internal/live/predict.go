...
//	0.0714    2026-08-15    41회차 추정 미체결율 7.3% 에 기대 정한 값.
//	0.14      2026-08-16    사용자 결정. p_up ≥ 0.57 또는 ≤ 0.43.
//	0.10      2026-08-17~  크기를 확신도에 비례시키면서 내렸다. 아래 참고.
//
// # 왜 다시 내렸나 — 크기가 문턱의 일을 대신한다
//
// 0.14 는 "여기 아래는 여유가 얇으니 아예 걸지 않는다" 였다. 크기가 균등일
// 때는 그 방법밖에 없다 — 얇은 회차에 같은 돈을 걸면 두꺼운 회차의 수익을
// 갉아먹는다. 2026년으로 재면 문턱만 0.10 으로 내리고 균등으로 걸 때 수익이
// +3,268 → +636 으로 무너진다.
//
// [risk.StakeTarget] 이 그 문제를 크기로 푼다. conf 0.10~0.12 구간은 교정
// 승률 54.54% 로 손익분기(54.01%) 바로 위이고, 켈리가 최고 칸의 7.9% 밖에
// 안 된다 — 그만큼만 건다. 그렇게 하면 같은 총 명목으로 회차가 3배 늘고
// 낙폭이 줄어든다(2026: 최대인출 1,229 → 966):
//
...
//
// 3년 통합 샤프는 균등 배분(0.98)보다 높은 1.14 다.
//
// # 문턱과 크기표는 같은 사실의 두 표현이다
//
// [risk.ConfidenceWeight] 의 첫 칸이 0.10 인 것이 우연이 아니다. 그 아래는
// 교정 승률이 손익분기 미만이라 켈리가 0 이 되고, 문턱이 없어도 크기가 0 이
// 된다. **둘 중 하나만 고치면 안 된다** — 이 값을 내리려면 크기표의 첫 칸도
// 같이 내려야 하고, 그러려면 그 구간의 승률을 다시 재야 한다.
//
// 실제로 걸리는 문턱은 [risk.MaxStakeUSD] 가 정한다. 지금 값(5 USDT)에서
// conf 0.14 아래 두 칸의 목표는 0.40·0.95 USDT 로 최소 주문 미만이라,
// **실효 문턱은 0.14** 다. 상한을 올리면 그만큼 내려간다.
//
// # 아래 0.14 시절의 기록은 그대로 둔다
//
// 손익분기 j* 계산의 근거이고, 그 계산은 지금도 유효하다.
//
// # 왜 올렸나 — 미체결율을 실측하니 손익분기가 훨씬 높았다
//
// 0.0714 은 미체결율 7.3% 라는 **41회차 추정치** 위에 서 있었다. 그 값이
// 틀리면 문턱도 틀린다는 것은 그때도 적어 두었다(아래 "기대는 가정" 참고).
//
// 2026-08-15 에 오더북 원본 1,406회차(07-29~08-06)로 그 값을 다시 쟀다:
//
//	P(체결 | 우리가 이길 회차)   85.5%
//	P(체결 | 우리가 질 회차)     99.9%
//	주수 가중 평균 진입가        0.4937
//
// **질 회차는 거의 100% 체결되고 이길 회차만 14.5% 가 빠진다.** 역선택이
// 있다는 것은 알고 있었지만 크기가 이 정도였다. 손익분기 판정 승률은
//
//	j* = P(체결|패)·px / ( P(체결|패)·px + P(체결|승)·(0.98−px) )
//	   = 0.999·0.4937 / (0.999·0.4937 + 0.855·0.4863) = 54.24%
//
// 이고, 이전에 쓰던 52.70% 보다 1.54%p 높다. 2024년 이후 문턱별 판정 승률은
//
//	문턱 0.0714   56만 → 55.5%    여유 +1.3%p
//	문턱 0.10     28,242회차 54.87%   여유 +0.6%p
//	문턱 0.12     16,643회차 55.68%   여유 +1.4%p
//	문턱 0.14      9,860회차 56.19%   여유 +1.9%p
//	문턱 0.20      2,120회차 58.07%   여유 +3.8%p
//
// 로, 문턱을 올릴수록 여유가 커진다. 회차 수는 반대로 줄어 총합은 완만한
// 봉우리를 그리는데, 자본 1,000 USDT · 낙폭 예산 35% 기준으로 0.14 부근이
// 샤프 2.07 로 가장 나았다(0.22 는 1.97, 0.10 은 그 아래).
//
// # 이 값이 기대는 가정 — 여전히 8일 표본이다
//
// 위 세 값은 모두 오더북 8.2일에서 나왔다. 특히 P(체결|승) 85.5% 는 저확신
// 회차까지 포함한 값이고, conf ≥ 0.10 구간만 보면 78.2% 로 떨어진다(n=101,
// 차이 1.7σ — 유의하지 않지만 방향은 일관적이다). 78.2% 라면 j* 가 57.46%
// 로 뛰어 이 문턱도 적자가 된다.
//
// `cmd/gld91/recorder.go` 가 그 표본을 계속 쌓는다. 다음에 이 값을 손볼 때는
// 추정이 아니라 그 기록으로 다시 재라.
